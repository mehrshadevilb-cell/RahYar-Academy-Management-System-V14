...
async def admin_products_list(callback: CallbackQuery, db):
    ...
    await callback.message.edit_text(
        "📚 محصولات:\n(✅ فعال / 🚫 غیرفعال)",
        reply_markup=admin_products_keyboard(courses),
    )
...
async def admin_product_toggle(callback: CallbackQuery, db):
    ...
    admin_log_service.log(
        db, callback.from_user.id, admin_actions.PRODUCT_TOGGLE,
        f"محصول «{course.title}» به وضعیت {status} تغییر کرد",
    )
...
async def admin_product_price_start(
    ...
    await callback.message.answer(
        f"قیمت جدید «{course.title}» را به تومان و فقط به‌صورت عدد بفرستید:"
    )
...
async def admin_product_price_set(
    message: Message,
    state: FSMContext,
    db,
):
    ...
    admin_log_service.log(
        db, message.from_user.id, admin_actions.PRODUCT_PRICE_CHANGE,
        f"قیمت «{course.title}» از {old_price:,} به {course.price:,} تومان تغییر کرد"
        if old_price is not None
        else f"قیمت «{course.title}» به {course.price:,} تومان تغییر کرد",
    )

    await message.answer(
        f"✅ قیمت «{course.title}» به {course.price:,} تومان تغییر کرد.",
...
async def admin_spotplayer_list(callback: CallbackQuery, db):
    ...
    await callback.message.edit_text(
        text,
        reply_markup=spotplayer_courses_keyboard(product_id, records),
    )
...
async def admin_spotplayer_toggle(callback: CallbackQuery, db):
    ...
    admin_log_service.log(
        db, callback.from_user.id, admin_actions.SPOTPLAYER_COURSE_TOGGLE,
        f"وضعیت کد SpotPlayer «{record.spotplayer_course_id}» تغییر کرد "
        f"({'فعال' if record.enabled else 'غیرفعال'})",
    )

    records = product_integration_service.get_spotplayer_courses(db, record.product_id)

    await callback.message.edit_reply_markup(
        reply_markup=spotplayer_courses_keyboard(record.product_id, records)
    )
...
async def admin_spotplayer_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "کد دوره (course id) SpotPlayer را بفرستید:"
    )
...
async def admin_spotplayer_add_id(message: Message, state: FSMContext):
    ...
    await message.answer(
        "نام نمایشی این دوره را بفرستید (یا برای رد کردن «-» بفرستید):"
    )
...
async def admin_spotplayer_add_name(message: Message, state: FSMContext, db):
    ...
    product_integration_service.add_spotplayer_course(
        db, product_id, spotplayer_course_id, course_name
    )

    admin_log_service.log(
        db, message.from_user.id, admin_actions.SPOTPLAYER_COURSE_ADD,
        f"کد SpotPlayer «{spotplayer_course_id}» به محصول شماره {product_id} اضافه شد",
    )

    records = product_integration_service.get_spotplayer_courses(db, product_id)

    await message.answer(
        "✅ کد دوره SpotPlayer ثبت شد.",
        reply_markup=spotplayer_courses_keyboard(product_id, records),
    )
...
async def admin_channels_list(callback: CallbackQuery, db):
    ...
    await callback.message.edit_text(
        text,
        reply_markup=telegram_channels_keyboard(product_id, records),
    )
...
async def admin_channel_toggle(callback: CallbackQuery, db):
    ...
    admin_log_service.log(
        db, callback.from_user.id, admin_actions.CHANNEL_TOGGLE,
        f"وضعیت کانال «{record.name}» تغییر کرد "
        f"({'فعال' if record.enabled else 'غیرفعال'})",
    )
    ...
    await callback.message.edit_reply_markup(
        reply_markup=telegram_channels_keyboard(record.product_id, records)
    )
...
async def admin_channel_add_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "نام کانال را بفرستید (مثلاً «فایل‌ها» یا «Record»):"
    )
...
async def admin_channel_add_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        "شناسه (chat_id) عددی کانال را بفرستید (ربات باید ادمین کانال با دسترسی "
        "ساخت لینک دعوت باشد):"
    )
...
async def admin_channel_add_chat_id(message: Message, state: FSMContext, db):
    ...
    admin_log_service.log(
        db, message.from_user.id, admin_actions.CHANNEL_ADD,
        f"کانال «{channel_name}» به محصول شماره {product_id} اضافه شد",
    )
...
async def admin_pending_list(callback: CallbackQuery, db):
    ...
    await callback.message.edit_text(
        f"💳 {len(pending)} پرداخت در انتظار بررسی:",
        reply_markup=admin_back_button(),
    )
...
async def admin_card_add_holder(message: Message, state: FSMContext, db):
    ...
    admin_log_service.log(
        db, message.from_user.id, admin_actions.PAYMENT_CARD_ADD,
        f"کارت پرداخت جدید به نام «{card_holder}» ثبت و فعال شد",
    )

    await state.clear()

    await message.answer(
        "✅ کارت جدید ثبت و فعال شد.",
        reply_markup=admin_back_button(),
    )

...
def _extract_name(text: str, phone: str | None) -> str | None:
    ...
    stop = {"هنرجو", "دانشجو", "داره", "دارد", "دیگه", "دیگر", "تکمیل", "دوره", "آنلاین"}
...
def parse_enrollment_chat(text: str) -> ChatEnrollmentDraft:
    ...
    if "ماهانه" in raw or "ماهیانه" in raw:
        draft.plan = "monthly"
    elif "ترم" in raw or (draft.remaining_sessions and draft.remaining_sessions >= 8):
        draft.plan = "term"
    if not draft.phone:
        draft.warnings.append("شماره موبایل پیدا نشد")
    if not draft.course_hint:
        draft.warnings.append("نوع کلاس پیدا نشد")

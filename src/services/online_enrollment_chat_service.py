"""Apply chat-parsed online enrollment drafts."""
...
class OnlineEnrollmentChatService:
    ...
    def format_preview(self, draft: ChatEnrollmentDraft, course: OnlineCourse | None, user: User | None) -> str:
        sessions = draft.remaining_sessions if draft.remaining_sessions is not None else draft.weeks
        lines = [
    ...
    def apply(self, db: Session, draft: ChatEnrollmentDraft) -> ApplyResult:
        ...
        if not course:
            return ApplyResult(False, "کلاس آنلاین مطابق متن پیدا نشد.")
        ...
        admin_notes = " | ".join(
            b for b in (draft.schedule_note(), "ثبت از چت ادمین") if b
        )[:500]
        if existing:
            ...
            return ApplyResult(
                True,
                f"✅ ثبت‌نام «{user.full_name}» در «{course.name}» به‌روز شد.\nجلسات: {enrollment.remaining_sessions}",
                enrollment.id,
            )

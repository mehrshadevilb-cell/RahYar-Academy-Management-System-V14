...
- the original request vs outcome summary

Persists the last finished run under .ai-agent/last_activity.json when a
repo path is available (best-effort; never raises to callers).
"""
...
@dataclass
class AgentActivity:
    kind: str = "idle"  # idle | analyze | implement
    mode: str = ""
    request: str = ""
...
class ActivityTracker:
    ...
    def start(self, *, kind: str, mode: str, request: str) -> None:
        with self._lock:
            ...
            self._current.steps.append(f"شروع شد · نوع={kind} · حالت={mode or '—'}")
    ...
    def tool(self, name: str, arg: str = "") -> None:
        with self._lock:
            ...
            self._current.steps.append(f"🧰 استفاده از ابزار {name}" + (f" → {arg[:80]}" if arg else ""))

    def files(self, paths: list[str]) -> None:
        with self._lock:
            self._current.files_changed = list(paths)[:30]
            if paths:
                self._current.steps.append(f"📝 فایل‌ها: {', '.join(paths[:8])}")

    def branch(self, name: str) -> None:
        with self._lock:
            self._current.branch = name
            self._current.steps.append(f"🌿 branch: {name}")
        ...
        *,
        ...
        with self._lock:
            ...
            self._current.steps.append(
                "✅ تمام شد" if success else f"❌ ناموفق: {(error or outcome)[:120]}"
            )
    ...
    def report_text(self) -> str:
        ...
        if current.status == "running":
            ...
            lines.append(f"درخواست شما:\n{(current.request or '—')[:500]}")
            lines.append("")
            ...
            if current.tools_used:
                lines.append("")
                lines.append("ابزارها: " + ", ".join(current.tools_used[-8:]))
            lines.append("")
            lines.append("می‌توانید چند ثانیه بعد دوباره همین دکمه را بزنید.")
            return "\n".join(lines)

        target = last if last.status in {"success", "failed"} else current
        ...
        status_icon = "✅" if target.status == "success" else "❌" if target.status == "failed" else "⚪"
        ...
        if target.started_at and target.finished_at:
            lines.append(f"مدت: ~{int(target.finished_at - target.started_at)} ثانیه")
        lines.append("")
        ...
        if target.status == "success":
            lines.append("بله — Agent کار را تمام کرد (جزئیات زیر).")
        ...
        if target.steps:
            lines.append("")
            lines.append("مراحل انجام‌شده:")
            for i, step in enumerate(target.steps[-20:], start=1):
                lines.append(f"{i}. {step}")

        if target.tools_used:
            lines.append("")
            lines.append("ابزارها: " + ", ".join(target.tools_used[-10:]))
...
# Process-wide singleton (one bot worker)

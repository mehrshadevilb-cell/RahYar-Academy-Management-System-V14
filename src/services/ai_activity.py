...
@dataclass
class AgentActivity:
    kind: str = "idle"  # idle | analyze | implement | consult
    mode: str = ""
    request: str = ""
...
class ActivityTracker:
    ...
    def tool(self, name: str, arg: str = "") -> None:
        with self._lock:
            ...
            self._current.steps.append(
                f"🧰 استفاده از ابزار {name}"
                + (f" → {arg[:80]}" if arg else "")
            )
    ...
    def finish(
        ...
        *,
        ...
        with self._lock:
            ...
            if cancelled:
                self._current.steps.append("🛑 لغو شد توسط کاربر")
            elif success:
    ...
    def report_text(self) -> str:
        ...
        if target.status in {"idle", ""} and not target.request:
            ...
            lines.append("بعد از Assistant / Debug / Fix / Feature این بخش پر می‌شود.")
            return "\n".join(lines)
